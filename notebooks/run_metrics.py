...
def do_stuff():
    ...
    sortWQS.to_csv("Results/all_batch1_workers_withAnnotationQuality_noOffsets.csv", index=False)
    sortAQS.to_csv("Results/all_batch1_annotations_withAnnotationQuality_noOffsets.csv", index=False)
    
    rows = []
    header = list(sortUQS['units'])
    header.extend(["event-event_pair", "no_of_annotations", "event-event_pair_final_score", "event-event_pair_initial_score"])

    remove_indices = [12, 17, 19]
    ...
    with open('Results/all_batch1_results_withAnnotationQuality_noOffsets.csv', 'w') as csvfile:
