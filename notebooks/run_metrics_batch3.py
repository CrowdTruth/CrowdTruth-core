...
crowd_noOffset = pd.read_csv("crowdflower/causal_relation_noOffset_lemma_all_batch3.csv")
...
def do_stuff():
    pre_processed_results = processFile(root=".", directory="", filename="crowdflower/causal_relation_noOffset_lemma_all_batch3.csv", config=config)
    ...
    sortWQS.to_csv("Results/all_batch3_workers_withAnnotationQuality_noOffsets.csv", index=False)
    sortAQS.to_csv("Results/all_batch3_annotations_withAnnotationQuality_noOffsets.csv", index=False)
    ...
    with open('Results/all_batch3_results_withAnnotationQuality_noOffsets.csv', 'w') as csvfile:
